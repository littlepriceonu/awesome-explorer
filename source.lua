local EmbeddedModules = {
    ...
    local function main()
        ...
        Explorer.InitRightClick = function()
            ...
            context:Register("VIEW_CONNECTIONS",{Name = "View Connections", IconMap = Explorer.ClassIcons, Icon = 34, OnClick = function()
                ...
                for _, Selection in ipairs(Selected) do
                    ...
                    if isa(node.Obj,"RemoteEvent") then
                        DoCheck = true
                    ...
                    if DoCheck then
                        ...
                        rconsoleprint("\nConnections For " .. '"')
                        rconsoleprint("@@BLUE@@")
                        rconsoleprint(node.Obj.Name)
                        rconsoleprint("@@CYAN@@")
                        rconsoleprint('"' .. " (" .. SignalName .. ")\n")
                        for i, connection in ipairs(getconnections(node.Obj[SignalName])) do 
                            ...
                            local ScriptNameString
                            local FunctionLineString
                            local FunctionNameString

                            local ScriptName = tostring(debug.getinfo(connection.Function, "s")[1])
                            local FunctionLine = tostring(debug.getinfo(connection.Function, "l")[1])
                            local FunctionName = tostring(debug.getinfo(connection.Function, "n")[1])

                            ScriptNameString = ScriptName
                            FunctionLineString = FunctionLine
                            FunctionNameString = '"' .. FunctionName .. '"'


                            if ScriptName == "nil" then
                               ScriptNameString = "unknown" 
                            end

                            if FunctionLine == "nil" then
                                FunctionLineString = "unknown"
                            end

                            if FunctionName == "nil" then
                                FunctionNameString = "Name Unknown"
                            end

                            rconsoleprint("@@RED@@")
                            rconsoleprint(tostring(i) .. ": Script = ".. ScriptNameString .. ":" .. FunctionLineString .. " | Function = "  .. FunctionNameString.. " (" .. tostring(connection.Function) .. ")\n")
                        end

                        if not hasConnections then
                            rconsoleprint("@@RED@@")
                            rconsoleprint("No Connections!\n")
                        end
